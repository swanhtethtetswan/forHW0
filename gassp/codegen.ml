...
let translate (globals, functions) =
  ...
  let the_module = L.create_module context "Gasp" in
  ...
  (* Define each function (arguments and return type) so we can 
     call it even before we've cre/;ated its body *)
  ...
  let build_function_body fdecl =
    ...
    | A.Decr -> L.build_neg
      ...
      (* Print bools*)
      | SCall ("printb", [e]) -> L.build_call printf_func [| bool_format_str ; (expr builder e) |] "printb" builder

      (* print floats *)
      | SCall ("printf", [e]) -> L.build_call printf_func [| float_format_str ; (expr builder e) |] "printf" builder

      (* print chars *)
      | SCall ("printc", [e]) -> L.build_call printf_func [| char_format_str ; (expr builder e) |] "prints" builder

      (* print strings *)
      | SCall ("prints", [e]) -> L.build_call printf_func [| string_format_str ; (expr builder e) |] "prints" builder

      (* printbig *)
      | SCall ("printbig", [e]) -> L.build_call printbig_func [| (expr builder e) |] "printbig" builder

      (* min *)
      | SCall ("min",[e1;e2]) -> L.build_call min_func [| (expr builder e1) ; (expr builder e2) |] "min" builder
	...
	SBlock sl -> List.fold_left stmt builder sl
      | SContinue -> builder 
      | SBreak -> builder 
      | SSwitch(e, _) -> ignore(expr builder e); builder 
